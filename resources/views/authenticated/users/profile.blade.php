    ...
    <div class="user_status p-3">
      ...
      <div class="">
        ...
        <div class="subject_inner">
          <form class="profile_subjects" action="{{ route('user.edit') }}" method="post">
            @foreach($subject_lists as $subject_list)
            <div class="profile_subjects_items">
              <label>{{ $subject_list->subject }}</label>
              <input type="checkbox" name="subjects[]" value="{{ $subject_list->id }}">
            </div>
            @endforeach
            <input type="submit" value="登録" class="btn btn-primary profile_subjects_btn">
